...
M(6,1) = params(4);     % Infected humans (Acumulative)
...
for i = domain(1):domain(2)
   ...
   if ins.action==1
    ...
    if ins.action==1
        %Value of mosquito and human Aleph
        aleph_m = 1+(v_m*M(2,i))/(M(2,i)+M(1,i));
        aleph_h = 1+(v_h*(M(4,i)+M(5,i)))/(M(4,i)+M(3,i)+M(5,i));
    else 
        %Value of mosquito and human Aleph
         aleph_m = 1+(v_m*(1-ins.Num(i))*M(2,i))/(M(2,i)+M(1,i));
         aleph_h = 1+(v_h*(1-ins.Nuh(i))*(M(4,i)+M(5,i)))/(M(4,i)+M(3,i)+M(5,i)); 
    end
    ...
    if (M(3,i)<1)
        ...
        G_mh = 1-(1-(1/M(3,i))^aleph_m)^(z*M(2,i)*(M(3,i)/M(8,i))); % Prob of interaction Sus Human - Inf mosquito
    end
    ...
    M(6,i+1) = M(6,i) + M(3,i)*beta_mh*G_mh*(1-mu_h); %Acumulative infected humans 
    % Total population
    M(7,i+1) = sum(M(1:2,i));
    M(8,i+1)= sum(M(3:5,i));
    % Store Aleph Values
